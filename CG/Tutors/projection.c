...
{
    if (!pmodel) {
        pmodel = glmReadOBJ("../data/obj/al.obj");
        if (!pmodel) exit(0);
        glmUnitize(pmodel);
...
void main_keyboard(unsigned char key, int x, int y)
{
	switch (key)
	{
		...
		case 'p':
		  mode = PERSPECTIVE;
		  changeWindowTitle("Projection - Using gluPerspective");
		break;
		case '3':
		case 'o':
		  mode = ORTHO;
		  changeWindowTitle("Projection - Using glOrtho");
		break;
		case '4':
		case 'f':
		  mode = FRUSTUM;
		  changeWindowTitle("Projection - Using glFrustum");
		break;
		case 'r':
		case '5':
		  changeWindowTitle("Projection - Resetting parameters");
...
void showMenu()
{
	...
	printf("Press 2 to use gluPerspective command\n");
	printf("Press 3 to use glOrtho command\n");
	printf("Press 4 to use glFrustum\n");
	printf("Press 5 to Reset parameters\n");
