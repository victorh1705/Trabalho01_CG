...
{
    if (!pmodel) {
        pmodel = glmReadOBJ("../data/obj/f-16.obj");
        if (!pmodel) exit(0);
        glmUnitize(pmodel);
...
screen_reshape(int width, int height)
{
    ...
    if (clear)
        glClearColor(color[0].value, color[1].value, color[2].value, color[3].value);
    else
...
void main_keyboard(unsigned char key, int x, int y)
{
    ...
    case '2':
        mode = GL_LINEAR;
		  changeWindowTitle("Fog - Using GL_LINEAR");
	 break;
    case 'e':
    case '3':
        mode = GL_EXP;
		  changeWindowTitle("Fog - Using GL_EXP");
    break;
    case 'x':
    case '4':
        mode = GL_EXP2;
		  changeWindowTitle("Fog - Using GL_EXP2");
    break;
    case 'c':
    case '5':
        clear = !clear;
		  changeWindowTitle("Fog - Clear color Toggle");
    break;
    case 'r':
    case '6':
        color[0].value = 0.7;
        ...
        fstart.value = 0.5;
        fend.value = 2.0;
		  changeWindowTitle("Fog - Reset Parameters");
        break;
    case 27:
        exit(0);
    }
...
void showMenu()
{
	...
	printf("Press 2 to set GL_LINEAR fog\n");
	printf("Press 3 to set GL_EXP fog\n");
	printf("Press 4 to set GL_EXP2 fog\n");
	printf("Press 5 to clear color toggle\n");
